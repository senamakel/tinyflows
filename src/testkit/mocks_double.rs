//! The call log ([`CapCall`]/[`CallOutcome`]/[`CallLog`]) and [`Double`], the
//! single type that implements every capability trait for
//! [`MockCaps`](super::MockCaps) by consulting its rules, recording what
//! happened, and answering.
//!
//! Split out of `mocks.rs` to keep that file under the repository's
//! line-length limit; `Double` and the log it writes to are one cohesive
//! concern (every trait impl below ends by writing a [`CapCall`]), so they
...
use super::{MockCaps, capability, glob_matches};
