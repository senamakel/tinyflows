...
`approval` node and its config does not set an explicit `request_id`: the
review's `request_id` defaults to `"<run id>:<node id>"`, so with neither a
configured `request_id` nor a run id, the node has no identity it can trust
and refuses rather than guessing one. Use a **server-generated** id, never a
caller-supplied field — it is the key reviews are de-duplicated on.
